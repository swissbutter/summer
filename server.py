...
@app.route("/api/check", method="POST")
def check_spelling():
    ...
    try:
        ...
        for chunk in chunks:
            ...
            if not res_json or "error" in res_json.get("message", {}):
                # Fallback: treat this chunk as plain text
                final_tokens.append({
                    "text": chunk,
                    "is_err": False,
                    "err_id": None,
            ...
            # Map chunk tokens and errors to global list
            chunk_err_idx = 0
            for t in c_tokens:
...
if __name__ == "__main__":
    ...
    print(f"Starting Interactive Spell Inspector server on http://0.0.0.0:{port}")
